---
title: "GPU Programming Introduction (GPU 프로그래밍 입문)"
date: 2026-02-22T12:02+09:00
tags:
  - graphics/gpu-architecture
summary: "Georgia Tech CS8803 Module 3 — CUDA 프로그래밍 기초, 블록/스레드 계층, 공유 메모리, 점유율"
order: 3
---
...
CUDA는 스레드를 블록으로 묶는 계층적 실행 구조를 도입한다. 스레드가 **블록(block)** 으로 그룹화되고, 블록들이 동시에 실행된다.
...
| **텍스처 메모리(Texture)** | 읽기 전용 | 특수 저장 공간 |
...
- SM당 워프가 많을수록 스레드 전환을 통한 **지연 시간 은닉(latency hiding)** 이 효과적이다
- 예외 사례는 이후 강의에서 다룬다
...
실제 연산 영역보다 더 많은 스레드를 생성하여, 경계 스레드는 경계 데이터를 로드하고 내부 스레드만 실제 연산을 수행한다.
