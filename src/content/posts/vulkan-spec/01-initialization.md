...
date: 2026-02-22T14:01+09:00
tags:
  - vulkan/spec
summary: "Vulkan을 쓰기 전 반드시 해야 할 두 가지 — 커맨드 함수 포인터 로딩과 VkInstance 생성."
order: 1
---
...
Vulkan 커맨드는 **정적 링킹으로 반드시 제공되지 않는다.** 플랫폼에 따라 동적으로 로딩해야 할 수 있다.
...
`vkGetInstanceProcAddr`로 얻은 함수 포인터는 **디스패치 코드를 거칠 수 있음** (여러 GPU가 있을 때 어떤 디바이스로 보낼지 판단하는 레이어). `vkGetDeviceProcAddr`로 얻으면 이 오버헤드를 제거할 수 있다.
...
**레이어 로딩 순서**: 배열의 첫 번째가 앱에 가장 가깝고, 마지막이 드라이버에 가장 가까움.
