...
Vulkan 커맨드는 **정적 링킹으로 반드시 제공되지 않는다.** <sup class="fn-ref"><a href="/log/2026-02-16-1100/#pfn-접두사의-의미">[1]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-16-1100/#pfn-접두사의-의미">PFN 접두사의 의미</a></span><span class="fn-desc">PFN = Pointer to FunctioN. C에서 함수 포인터 타입을 typedef로 정의할 때 쓰는 네이밍 컨벤션.</span></span></sup> 플랫폼에 따라 동적으로 로딩해야 할 수 있다.
...
`vkGetInstanceProcAddr`로 얻은 함수 포인터는 **디스패치 코드를 거칠 수 있음** <sup class="fn-ref"><a href="/log/2026-02-16-1100/#디스패치dispatch란">[2]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-16-1100/#디스패치dispatch란">디스패치(Dispatch)란?</a></span><span class="fn-desc">어디로 보낼지 판단해서 라우팅하는 것. Instance 레벨 함수 포인터는 디스패치 코드를 거치고, Device 레벨은 직통.</span></span></sup> (여러 GPU가 있을 때 어떤 디바이스로 보낼지 판단하는 레이어). `vkGetDeviceProcAddr`로 얻으면 이 오버헤드를 제거할 수 있다.
...
**레이어 로딩 순서**: 배열의 첫 번째가 앱에 가장 가깝고, 마지막이 드라이버에 가장 가까움. <sup class="fn-ref"><a href="/log/2026-02-16-1100/#레이어-배열-순서">[3]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-16-1100/#레이어-배열-순서">레이어 배열 순서</a></span><span class="fn-desc">배열 첫 번째가 앱에 가장 가깝고, 마지막이 드라이버에 가장 가까움. 순서에 따라 호출 체인 위치가 달라진다.</span></span></sup>
...
### 2.6 인스턴스 파괴 — vkDestroyInstance <sup class="fn-ref"><a href="/log/2026-02-16-1000/#pallocator는-cpu-측-메모리다">[4]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-16-1000/#pallocator는-cpu-측-메모리다">pAllocator는 CPU 측 메모리다</a></span><span class="fn-desc">pAllocator는 GPU 메모리가 아닌 CPU 메모리 할당을 커스터마이징하는 것. 드라이버 내부 메타데이터 할당에 관여.</span></span></sup>
