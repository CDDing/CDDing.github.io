...
각 스레드는 스레드 ID로 결정되는 `a[idx]`와 `b[idx]`에 접근하여 서로 다른 벡터 요소에 대해 동작한다.<sup class="fn-ref"><a href="/log/2026-02-22-1000/#spmd-데이터-인덱싱과-메모리-sm-매핑-시각화">[1]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-22-1000/#spmd-데이터-인덱싱과-메모리-sm-매핑-시각화">SPMD 데이터 인덱싱과 메모리-SM 매핑 시각화</a></span><span class="fn-desc">벡터 덧셈 예제로 블록→SM 배정, 인덱스 계산, 메모리 계층 접근 경로를 시각화한 노트.</span></span></sup>
...
| 메모리 유형 | 범위 | 특징 |
|------------|------|------|
| **공유 메모리(Shared)** | CUDA 블록 내 | 온칩, 빠른 접근<sup class="fn-ref"><a href="/log/2026-02-22-1000/#온칩on-chip-vs-오프칩off-chip-메모리">[3]</a><span class="fn-tooltip"><span class="fn-title"><a href="/log/2026-02-22-1000/#온칩on-chip-vs-오프칩off-chip-메모리">온칩 vs 오프칩 메모리</a></span><span class="fn-desc">온칩=GPU 다이 내부(SRAM), 오프칩=별도 메모리 칩(DRAM). 속도·용량·비용 트레이드오프.</span></span></sup> |
