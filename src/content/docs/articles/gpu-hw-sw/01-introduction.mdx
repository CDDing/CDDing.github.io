...
- **CPU**: 단일 스레드 애플리케이션에 맞추어져 최대 속도를 목표로 한다. 정확한 예외 처리(precise exception)를 우선시하며, 이는 프로그램 정확성과 디버깅에 중요하다. <sup>[📝](/log/misc/gpu_architecture/#precise-exception--cpu-vs-gpu "Precise Exception — CPU vs GPU")</sup>
...
- **GPU**: 가속기 모드로 동작하며, 드라이버가 하나의 ISA에서 특정 하드웨어용 ISA로 코드를 변환한다. PTX는 공개 가상 ISA로 도입되었다. <sup>[📝](/log/misc/gpu_architecture/#가속기-모드와-ptx "가속기 모드와 PTX")</sup>
...
하나의 SM 내부에는 명령어 캐시, 디코더, 공유 메모리, 여러 스트림 프로세서가 포함된다. <sup>[📝](/log/misc/gpu_architecture/#sm-sp-워프의-관계 "SM, SP, 워프의 관계")</sup>
