...
<div class="note-date">2026-02-22</div>
...
방향은 맞지만 한 가지 보정이 필요하다. 워프는 **SP(하드웨어)의 그룹이 아니라 스레드(소프트웨어)의 그룹**이다. <sup class="fn-ref"><a href="/notes/gpu-architecture/#gpu-스레드의-본질--실행-컨텍스트">[1]</a><span class="fn-tooltip"><span class="fn-title"><a href="/notes/gpu-architecture/#gpu-스레드의-본질--실행-컨텍스트">GPU 스레드의 본질 — 실행 컨텍스트</a></span><span class="fn-desc">스레드는 함수가 아니라 실행 인스턴스(PC + 레지스터 + ID). 레지스터 파일에 상주하므로 워프 전환 비용이 0.</span></span></sup>

### 정확한 계층 구조

- **SM (Streaming Multiprocessor)**: SP들 + 워프 스케줄러 + 레지스터 파일 + 공유 메모리를 묶은 실행 단위. CPU 코어에 대응. <sup class="fn-ref"><a href="/notes/gpu-architecture/#레지스터-파일-vs-sp--분할되는-것은-무엇인가">[2]</a><span class="fn-tooltip"><span class="fn-title"><a href="/notes/gpu-architecture/#레지스터-파일-vs-sp--분할되는-것은-무엇인가">레지스터 파일 vs SP</a></span><span class="fn-desc">레지스터 파일은 스레드마다 파티션 분할. SP는 분할하지 않고 워프가 교대로 사용하는 공유 연산 유닛.</span></span></sup>
...
## 스텐실 연산 — 그래픽스 스텐실과 다른 개념

> **Q.** 병렬 프로그래밍에서 나오는 "스텐실 연산"은 그래픽스의 스텐실 버퍼와 같은 건가?

완전히 다른 개념이다. 이름만 같을 뿐 분야와 용도가 다르다.

### 수치 해석의 스텐실 (Stencil Computation)

격자(grid) 위의 각 요소를 갱신할 때, **고정된 패턴의 이웃 요소들**을 참조하여 새 값을 계산하는 연산 패턴이다. [수치 해석에서 "스텐실"이란 관심 지점과 관련된 노드 그룹의 기하학적 배치](https://en.wikipedia.org/wiki/Stencil_(numerical_analysis))를 의미한다.

```
2D 5-point 스텐실 예시:

        [N]
   [W]  [C]  [E]     →  C_new = (N + S + E + W) / 4
        [S]
```

이 패턴을 격자의 **모든 요소에 반복 적용**하는 것이 [반복 스텐실 루프(Iterative Stencil Loop)](https://en.wikipedia.org/wiki/Iterative_Stencil_Loops)이다. 열전달 시뮬레이션, 유체역학, Jacobi 반복법 등 PDE(편미분방정식) 수치 풀이의 핵심 패턴이다.

이름의 유래는 물리적 스텐실(구멍 뚫린 판)에서 왔다 — 격자 위에 고정된 모양의 "창"을 놓고, 그 안에 보이는 이웃들만 참조한다는 비유이다.

### 그래픽스의 스텐실 (Stencil Buffer/Test)

렌더링 파이프라인에서 **픽셀 단위로 렌더링을 마스킹**하는 메커니즘이다. 스텐실 버퍼에 저장된 정수값과 비교하여 픽셀을 그릴지 말지 결정한다. 그림자, 포탈, 아웃라인 효과 등에 사용된다.

### 비교

| | 수치 해석 스텐실 | 그래픽스 스텐실 |
|---|---|---|
| **분야** | HPC, 수치 해석, 과학 시뮬레이션 | 실시간 렌더링 |
| **의미** | 이웃 참조 패턴 (neighbor access pattern) | 픽셀 마스킹 테스트 |
| **대상** | 격자 데이터 전체 | 프레임버퍼 픽셀 |
| **GPU 활용** | 공유 메모리 + 경계 로딩으로 병렬화 | 고정 함수 파이프라인 단계 |

강의(CS8803)에서 말하는 스텐실은 **수치 해석 쪽**이다. CUDA에서 공유 메모리 활용의 대표 예제로 등장하는 이유는, 이웃 요소를 반복 참조하므로 [공유 메모리에 타일링하면 전역 메모리 접근을 크게 줄일 수 있기 때문](https://developer.nvidia.com/blog/finite-difference-methods-cuda-cc-part-1/)이다.
